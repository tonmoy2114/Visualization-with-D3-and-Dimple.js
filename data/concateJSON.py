...
arr = []
HR_limit = 84
with io.open('Steps_D2.json', encoding="utf-8") as json_file:
...
with io.open('HR_D2.json', encoding="utf-8") as json_file:
	data = json.load(json_file)
	for item in data:
		stress_value = 0
		if item['value'] > HR_limit:
			stress_value = (item['value'] - HR_limit) * valid_states_map[item['time']]
		output.write(item['time']+","+str(item['value'])+","+str(hashMap[item['time']])+","+str(stress_value)+",D2,\n")
	output.close()
